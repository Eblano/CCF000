...
public class Man : MonoBehaviour
{
    ...
    public float TuningOfTurnShooting = 0;
    ...
    void Start()
    {
        ...
        if (NewModel)
        {
            if (!bip) bip = transform.FindChild("mixamorig:Hips");
            if (!bip_collider) bip_collider = bip.FindChild("mixamorig:Spine");
            if (!bip_target) bip_target = bip_collider.FindChild("mixamorig:Spine1").FindChild("mixamorig:Spine2").FindChild("mixamorig:Neck");
        }
        /*
        ...
        audio_source = GetComponent<AudioSource>(); //gameObject.AddComponent(typeof(AudioSource)) as AudioSource;
    ...
    void Update()
    {
        ...
        else if (doing == "fall")
        {
            ...
            {
                fall_timer = 0;
                if(player)player_sc.MInput.ManController.ParachuteButton.SetActive(false);
                Move();
    ...
    public void Danger()
    {
        if (!cop)
        {
            ...
            {
                //player_sc.StartCoroutine(player_sc.WantedCoroutine()); //только для мед сити!
            }
            else
            {
                danger_timer = 0;

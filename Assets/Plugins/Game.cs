...
public class Game : MonoBehaviour
{
    ...
    public MiniMap MiniMapCameraSc;
    ...
    {
        int num = PlayerPrefs.GetInt("selchar");
        player = Players[num];
        player_sc.player = player;
        ...
        MiniMapCameraSc.PlayerTr = player;
        MiniMapCameraSc.PlayerTr = _camera;
    }
    ...
    {
        if (!_transform.GetComponent<Man>().player)
        {
            player_sc.CountKill += 1;
            player_sc.Rewarded += _transform.GetComponent<Man>().Rewarded;
        }
        MainInput MInput = MainInput.instance;

...
public class Player : MonoBehaviour
{
    ...
    private int TimeMatch = 0;

    public SpawnObjectsRandom SpawnObjectsScript;
    ...
    void LateStart()
    {
        ...
        weapon_script.WeaponGive(PlayerPrefs.GetInt("selwep")+1, false, 0, 0);
        //weapon_script.WeaponGive(20,false,0,0);

        //for (var i = 0; i < weapon.Length; i++)//add load weapon
        //{
        ...
        //if (PlayerPrefs.GetInt("parachute") == 1) man_script.GiveParachute();//add load parachute
            ...
            if (man_script.doing != "sit_vehicle")
            {
                ...
                if (fire)
                {
                    ...
                    if (Target != null)
                    {
                        float i = TargetCar ? 0.5f : 1.5f;
                        weapon_script.RotateTo(Quaternion.LookRotation((Target.position + Vector(0, i, 0)) - man_script.transform.position).eulerAngles.z,
                            Quaternion.LookRotation(Target.position - man_script.transform.position).eulerAngles.y + man_script.TuningOfTurnShooting);
                    }
                    if (fire_timer > 3 || man_script.doing == "go_to_vehicle")
                    {
                        fire_timer = 0;
    ...
    public void AimFunctions()
    {
        ...
        if(Target != null)
        {
            ...
            _camera.eulerAngles = Vector3.Lerp(Vector(_camera.eulerAngles.x, _camera.eulerAngles.y, _camera.eulerAngles.z), 
                Vector(temp_angle.x, temp_angle.y, temp_angle.z), Time.deltaTime);
        }
        else
        {
            TargetAim.gameObject.SetActive(false);
    ...
    public void ReloadedWeaponButton()
    {
    ...
    public IEnumerator ResultMatch()
    {
        while (true)
        {
            TimeMatch += 1;

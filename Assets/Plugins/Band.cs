...
public class Band : MonoBehaviour
{
    ...
    void Update()
    {
        if(StartAttack && !enemy)
        {
            enemy = game_sc.GetComponent<Player>().player;
        }
        ...
        {
            if (man_script_enemy == null) man_script_enemy = enemy.GetComponent<Man>();
            bot_script.other_doing = true;
    ...
    public void FindEnemy()
    {
        ...
        else if (fire)
        {
            ...
            if (StartAttack && !enemy)
            {
                enemy = game_sc.player;
            }
            bot_script.target_pos = enemy.transform.position;
